...
module.exports = async function deploy(config) {
	...
	try {
		...
		// 执行上传前脚本
		if (isValidArray(config.commandsBeforeUpload)) {
			...
			cmdSpinner.succeed('execute completely')
		}
		// 上传文件夹
		...
		// 执行上传后脚本
		if (isValidArray(config.commandsAfterUpload)) {
...
async function execCommands(cmds, remoteDir, spinner) {
	try {
		await ssh.execCommand(`cd ${remoteDir}`, { cwd: remoteDir })
		for (const cmd of cmds) {
			spinner.text = `execute ${cmd}`
			await ssh.execCommand(cmd, { cwd: remoteDir })
		}
	} catch (err) {
		console.log(err)
