...
module.exports = function validateDeployConfig(env) {
	...
	if (!fs.existsSync(deployConfigPath)) {
		log.error(
			'can not found deploy.config.js file, please execute init command first!'
		)
		process.exit(1)
	}
	// 检测configs[env]
	...
	if (config && Object.prototype.toString.call(config) === '[object Object]') {
		...
		// 判断密码和私钥是否至少有一个
		if (valid && !config.password && !config.privateKey) {
			log.error('there must be a password or privateKey')
			valid = false
		}
		// 判断dirctories或files中的目录或文件是否存在
		if (valid && isValidArray(config.dirctories)) {
			valid = config.dirctories.every(dir => {
				return fs.existsSync(path.join(localDir, dir))
			})
		}
		if (valid && isValidArray(config.files)) {
			valid = config.files.every(file => {
				return fs.existsSync(path.join(localDir, file))
			})
		}
		if (!valid) {
			process.exit(1)
